...
/**
 * Constructs Nodemailer transporter using configured SMTP environment variables.
 */
...
/**
 * Sends a cryptographic verification email.
 * If SMTP credentials are not configured, logs a clear server-side advisory without exposing secrets.
 */
export async function sendVerificationEmail(
  ...
  const fromAddress = process.env.SMTP_FROM || 'SecureShare Vault <no-reply@secureshare.local>';
        ...
        <table border="0" cellspacing="0" cellpadding="0">
          <tr>
            ...
            <td style="padding-left: 14px;">
              <h1 style="margin: 0; color: #ffffff; font-size: 20px; font-weight: 700; letter-spacing: -0.5px;">SecureShare</h1>
              <p style="margin: 2px 0 0 0; color: #bfdbfe; font-size: 12px; font-weight: 500;">Secure File Storage & Sharing</p>
            </td>
          </tr>
        </table>
      </td>
    ...
    <tr>
      <td style="padding: 32px;">
        ...
        <p style="margin: 0 0 24px 0; color: #475569; font-size: 14px; line-height: 1.6;">
          Thank you for creating an account with <strong>SecureShare</strong>. To activate your personal vault and access secure file storage, please verify your email address by clicking the button below:
        </p>
        ...
        <table border="0" cellspacing="0" cellpadding="0" style="margin: 28px 0;">
          <tr>
            <td align="center" style="border-radius: 12px; background-color: #2563eb;">
              <a href="${verificationUrl}" target="_blank" style="display: inline-block; padding: 14px 28px; font-size: 14px; font-weight: 600; color: #ffffff; text-decoration: none; border-radius: 12px;">
        ...
        <div style="background-color: #f1f5f9; border-left: 4px solid #2563eb; padding: 12px 16px; border-radius: 0 8px 8px 0; margin-bottom: 24px;">
          <p style="margin: 0; font-size: 12px; color: #334155; line-height: 1.5;">
            <strong>Expiry Information:</strong> This verification link expires in <strong>24 hours</strong>. If it expires, you can request a new one at the login screen.
          </p>
        </div>
        ...
        <p style="margin: 24px 0 0 0; border-top: 1px solid #f1f5f9; padding-top: 16px; color: #94a3b8; font-size: 12px; line-height: 1.5;">
          <strong>Security Notice:</strong> If you did not register for an account with SecureShare, you can safely ignore this email. No further action is required.
        </p>
      </td>
    </tr>
...
`;
  ...
  if (!transporter) {
    console.warn(
      `[EMAIL SERVICE ADVISORY] SMTP_HOST is not configured. Verification email was not sent. Configure SMTP_HOST, SMTP_PORT, SMTP_USER, and SMTP_PASSWORD before using email verification.`
    );
    return {
      sent: false,
      reason: 'SMTP is not configured; configure SMTP_HOST, SMTP_PORT, SMTP_USER, and SMTP_PASSWORD before delivering verification emails.',
    };
  }
  ...
  try {
    ...
    if (process.env.NODE_ENV !== 'production') {
      console.log(`[EMAIL SERVICE] Development verification URL: ${verificationUrl}`);
    }
    return { sent: true };
  } catch (err: any) {
